...
class SimulatedAnnealingOptimizer:
                ...
                if sorted_contributions and sorted_contributions[0][2] == 0:
                    ...
                    while attempts < 10:
                        ...
                        if new_group not in neighbor:
                            ...
                            new_coverage_ratio = sum(1 for cnt in new_coverage_count.values() if cnt >= self.f) / len(
                                self.j_subsets)
    ...
    def optimize(self, initial_temp=10.0, cooling_rate=0.995,
        ...
        if verbose:
            print(
                f"Initial solution: {len(current_solution)} combinations, coverage: {current_coverage:.2%}, energy: {current_energy}")
            print(f"Starting simulated annealing with temperature: {temperature}...")

        should_continue = self._update_progress(1,
                                                f"Initial solution: {len(current_solution)} combinations, coverage: {current_coverage:.2%}")
        if not should_continue:
        ...
        while temperature > stopping_temp and iterations < max_iterations:
            ...
            if iterations % 100 == 0:  # 每100次迭代更新一次进度，避免过于频繁
                should_continue = self._update_progress(progress,
                                                        f"Temperature: {temperature:.4f}, Best coverage: {best_coverage:.2%}, Groups: {len(best_solution)}")
                if not should_continue:
            ...
            if (energy_delta < 0) or (
                ...
                if best_delta < 0 or (best_delta == 0 and len(neighbor) < len(best_solution)):
                    ...
                    if verbose and improved_count % 10 == 0:
                        print(
                            f"Improvement: coverage {best_coverage:.2%}, groups: {len(best_solution)}, energy: {best_energy}, temp: {temperature:.4f}")

                    should_continue = self._update_progress(progress,
                                                            f"New best solution! Coverage: {best_coverage:.2%}, Groups: {len(best_solution)}")
                    if not should_continue:
                        return best_solution  # 如果用户暂停，返回当前最佳解
            ...
            if best_coverage == 1.0 and iterations - last_improvement > 1000:
                ...
                if temp_coverage == 1.0 and (temp_energy < best_energy or (
                        temp_energy == best_energy and len(temp_solution) < len(best_solution))):
                    ...
                    should_continue = self._update_progress(progress,
                                                            f"Optimized solution! Groups reduced to: {len(best_solution)}")
                    if not should_continue:
                        return best_solution  # 如果用户暂停，返回当前最佳解
            ...
            if iterations - last_improvement > 5000:
                ...
                should_continue = self._update_progress(progress,
                                                        f"Restarting search with best coverage: {best_coverage:.2%}")
                if not should_continue:
                    return best_solution  # 如果用户暂停，返回当前最佳解
        ...
        if verbose:
            ...
            print(
                f"Best solution: {len(best_solution)} combinations, coverage: {best_coverage:.2%}, energy: {best_energy}")
            print(f"Iterations: {iterations}, Accepted: {accepted}, Improved: {improved}")

        # 完成进度更新
        self._update_progress(100,
                              f"Optimization complete. Final solution has {len(best_solution)} groups with {best_coverage:.2%} coverage")
